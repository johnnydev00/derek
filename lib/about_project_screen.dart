...
class _AboutProjectScreenState extends State<AboutProjectScreen> {
  ...
  void getProjectInfo() async {
    ...
    final response = await http
        .get(Uri.https('derek.edus.kz', 'slim/index.php/api/getProject'));
    
    if (response.statusCode == 200) {
      List sources = json.decode(response.body);
      setState(() {
        aboutProject = sources[0]['answer_name_' + requestLang];
      });
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: aboutProject == '' ? Center(child: CircularProgressIndicator()) : SingleChildScrollView(
        child: Padding(
